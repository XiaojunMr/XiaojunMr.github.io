function workExpYearFac(h){
	...
	//s.K_P = function(){return false;};
	//s.html.onkeydown = s.K_P;
	s.showValue();
...
workExpYearFac.prototype = {
	...
	genYearHTML : function(){
		...
		for(var i=0;i<s.data.length;i++){
			if(s.data[i].length){
				...
				if(itemIndex>=col&&colIndex==0) html += '<tr><td colspan="'+col+'" class="line"><img src="http://my.zhaopin.com/images/blank.gif" width="1" height="1"></td></tr>';
	...
	genMonthHTML : function(){
		...
		for(var i=0;i<s.monthT[s.lang].length;i++){
			...
			val = i+1;
			colIndex = i%col;
			if(i>=col&&colIndex==0) html += '<tr><td colspan="'+col+'" class="line"><img src="http://my.zhaopin.com/images/blank.gif" width="1" height="1"></td></tr>';
			if(colIndex==0) html += '<tr>';
