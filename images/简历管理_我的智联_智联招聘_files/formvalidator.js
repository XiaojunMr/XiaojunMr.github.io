...
// 修改简历职位名称联想
// http://smart.zhaopin.com/hotword/jsonp?ip=127.0.0.1&client=my&S_HOT_FULL=jisuanji&S_HOT_TYPE=4&rows=2&callback=jquery&format=small 线上
//http://172.17.5.73:8080/hotword/jsonp?ip=127.0.0.1&client=my&S_HOT_FULL=jisuanji&S_HOT_TYPE=4&rows=2&callback=jquery&format=small  QA
$(document).ready(function(){
//var jobNamesArray = (language ==1 ? CONST_SUBJOBTYPE : CONST_SUBJOBTYPE_EN);
$("#customSubJobtype").bind("keyup",{requestURL:'http://smart.zhaopin.com/hotword/jsonp?ip=127.0.0.1&client=my&format=small',rows: 10},function(event){
	/*if(language == 2){
		requestURL.S_HOT_TYPE(5);
    }*/
    ...
    var paramData =(language == 2 ? 'S_HOT_TYPE=5' : 'S_HOT_TYPE=4')+'&S_HOT_FULL=' + encodeURIComponent(value) +'&rows='+event.data.rows;
    $.ajax({
    	type:'get',
    	...
    	success:function(data){
    		var d1 = data.results, dd = [];
    		if(!d1 || d1.length == 0){
    			$("#jobNameLenovo").html('').hide();
		        return;
    		}
            for(var k = 0; k < d1.length; k++){
            	var d2 = d1[k], jobCode = '', subCode = '';
            	if(d2.types.length > 0){
            		if(d2.types[0].jobType.length > 1){
            			jobCode = d2.types[0].jobType[0].code;
            			subCode = d2.types[0].jobType[1].code;
            		}
            	}
    			dd.push({
    				jobWord:d2.word,
    		...
    		for(var i = 0; i < data.length; i++){ 
				if(i == 0){
					ulStr+='<li jobTypeId="'+ data[i].jobCode + '" subJobTypeId="'+ data[i].subCode + '" class="current">'+data[i].jobWord+'</li>';
        		}else{
            		ulStr+='<li jobTypeId="'+ data[i].jobCode + '" subJobTypeId="'+ data[i].subCode + '">'+data[i].jobWord+'</li>';
       			 }        
			}
    		var jobNamesUl = $("#jobNameLenovo");
    		jobNamesUl.html(ulStr);
    		$("#jobNameLenovo li").bind('click',function(){
    			var t = $(this);
    			$("#customSubJobtype").val(t.text()).siblings('#SchJobType').val(t.attr('jobTypeId')).siblings('#subJobType').val(t.attr('subJobTypeId'));
    ...
    $("#customSubJobtype").bind("keydown",function(event){
    	var $inout = $(this), $ul_li = $("#jobNameLenovo li");
    	keyDown_UpDownEnter(event,$ul_li,function(){
    		setValueByEnterForComNam($inout, $ul_li);
    	})
    })
})
/*回车时给职位名称input赋值*/
    ...
    if($ul_li.length > 0){
        ...
        $ul_li.each(function(index, li){
            ...
            if($(li).hasClass('current')){
                $input.val($li.text()).siblings('#SchJobType').val($li.attr('jobTypeId')).siblings('#subJobType').val($li.attr('subJobTypeId'));
...
/*下拉列表上移动键盘的上下方向键时改变下拉列表项li的class*/
