...
//'########################################################################
if (typeof MZ_REG == "undefined" || !MZ_REG)
    var MZ_REG = {};
MZ_REG.HTML_class = {
	infoboxOk : 'msg_ok',
...
MZ_REG.checkMailByAJAX = function(mailInput){
	...
	if((status<2 || status==7 ) && status!=-1  ){
		...
		var data = 'userName=' + ua(mailInput.value) + '&t=' + new Date().getTime();
		//以上由 jianhuan.weng 修改 Task No: 90033
		submitCallback(data, 'http://i.zhaopin.com/LoginManager/VerifyUser', MZ_REG.handleCheckMailResponse, 'get');
	}
	else return;
}
	...
	if(window[mailVarName]){
		if(success){
			...
			if(!isNaN(errorIndex)){
                ...
                if(errorIndex == 0){document.getElementById("email").parentNode.className = "inputTexts" ;}
			}
		}
