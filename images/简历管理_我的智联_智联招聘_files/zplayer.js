...
(function () {
    commonFun = {
        ...
        getFloat:function(str){
            if (this.isObj(str)) {
                return parseFloat(str.toString().replace("px", ""));
            }
            else {
                return 0;
            }
...
/****************************经常通用拓展****************************/
//标签嵌入提示：
        htmlTip: function (title, $div, sureFun, canFun,initFun) {
            return this.init({
                type: 1,
                title: title,
                shade: 0.3,
                ...
                yes: function (index, layero) {
                    if (commonFun.isFunc(sureFun)) {
                        sureFun();
                    }
                    layer.close(index);
                },
                cancel: function (index) {
                    if (commonFun.isFunc(canFun)) {
                        canFun();
                    }
                },
				success:function(){
				   if (commonFun.isFunc(initFun)) {
                        initFun();
                    }
				}
            });
        ...
        //自定义参数
        //这个方法在老版里用的很多，以后尽量少用，如果用htmltip实现不了，就直接使用open，如果只是按钮的排版改变，可以使用备用里的commonLayer方法。
		showTipCommon:function(options){
        ...
        loadTip: function () {
          ...
          zpLayer.parm.currentIndex=index;   
        
        },
        //关闭某个弹出框
        closeLayer: function () {
            layer.close(zpLayer.parm.currentIndex);
        },
        //确定按钮，需调用layer.close(index);来关闭窗口
	    htmlTipFun: function (title, $div, sureFun, canFun,initFun) {
            return this.init({
                type: 1,
                title: title,
                shade: 0.3,
...
/****************************建议不使用****************************/
        //支持html键入以及DOM引入 2016-5-5 ccj
        //option参数说明:option.title弹层的头部，option.content弹层的内容,option.btnType弹层的底部的按钮类型，
        commonLayer: function (option) {
            var btnValue=option.btnType=="doubleBtn"?['确定', '取消']:option.btnType=="singleBtn"?['关闭']:false;
            return this.init({
                type: 1,
                title: option.title,
                shade: 0.3,
                ...
                btn: btnValue,
                yes: function (index, layero) {
                    if (commonFun.isFunc(option.sureFun)) {
                        option.sureFun();
                    }
                    layer.close(index);
                },
                cancel: function (index) {
                    if (commonFun.isFunc(option.canFun)) {
                        option.canFun();
                    }
                }
            });
        ...
        //普通消息提示:
        Tip: function (title, content, iconType) {
            return this.init({
                title: title,
                content: content,
                shade: 0.3,
        ...
        //确认提示：
        confirmTip: function (title, content, sureFun, canFun) {
            return this.init({
                type: 0,
                icon: 3,
                title: title,
        ...
        //定义两个以上按钮[或无按钮]
        htmlTipOption: function (title, $div, sureFun, canFun, options) {
            var defaults = {
                type: 1,
                title: title,
                shade: 0.3,
            ...
            $.extend(defaults, options);
            return this.init(defaults);
        },
        //弹出链接页面
        pageTip: function (title, url, width, heigt, allowScreen, successFun) {
            return this.init({
                type: 2,
                title: title,
                shade: 0.3,
                content: url,
                area: [width + "px", heigt + 'px'],
                success: successFun,
                maxmin: allowScreen
            });
        },
        //子页面关闭父页面
        childCloseTip: function () {
            var index = parent.layer.getFrameIndex(window.name);
            parent.layer.close(index);
        },
        //提示框：
        showTip: function (id, content) {
            return layer.tips(content, id, {
                tips: 2,
                shade: 0.3
            });
        },
        ...
        //3秒提示框
        whaitTip: function (content, iconId, closeFun) {
            layer.msg('有表情地提示', { icon: 6, time: 3000 }, function () {
                if (commonFun.isFunc(closeFun)) {
                    closeFun();
                }
            });
        },
        //关闭某个弹出框
        closeTip: function (index) {
            layer.close(index);
        },
